...
<table class="table table-list_grid_1" width="100%">
    <tbody>
        ...
        <tr>
            ...
            <td align="center">
                <span>
                    <a href="/{{ item['id'] }}" onClick="jsAsync.ShowModal(event, '<p>Подтвердите удаление</p>', '<p>Вы действительно хотите удалить запись {{ item['person_full_name'] }} {{ item['person_full_address'] }} ?</p>', 'Да', 'Нет', 'delete', '', '{{ item['id'] }}');">
                        Удалить
                    </a>(
                    <a href="#" title="Колличество связей с Кешом станций">{{ item['from_persons_cache'] }}</a>
                )</span>
            </td>
            <td align="center">
                <span>
